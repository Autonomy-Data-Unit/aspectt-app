<script lang="ts">
	...
	async function doSearch() {
		...
		matchingNames = data.matching_tool_names;
		total = data.total;
		loading = false;
	}
...
<svelte:head><title>Tools and equipment search - ASPECTT</title></svelte:head>

<div class="container">
	<h1 class="page-title">Tools and equipment search</h1>
	<p class="page-desc">Find occupations that use specific tools and equipment.</p>
	...
	{:else if searched}
		<div class="card">
			...
			{#if results.length === 0}
				<p class="no-results">No occupations found using tools matching "{query}"</p>
			{:else}
				<div class="results">
					{#each results as r}
						<a href="/occupation/{r.uk_soc_2020}" class="result-item">
							...
							<div class="matching-list">
								{#each r.matching_tools.slice(0, 8) as tool}
									<span class="match-tag">{tool}</span>
								{/each}
								{#if r.matching_tools.length > 8}
									<span class="more">+{r.matching_tools.length - 8}</span>
								{/if}
							</div>
						</a>
					{/each}
