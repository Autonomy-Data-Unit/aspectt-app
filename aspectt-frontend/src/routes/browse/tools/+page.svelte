<script lang="ts">
	import { getToolsBrowse, type TechBrowseItem } from '$lib/api/client';
	...
	async function load() {
		loading = true;
		const data = await getToolsBrowse(query, 200);
		items = data.tools_used;
		total = data.total;
		loading = false;
	}
...
<svelte:head><title>Browse tools and equipment - ASPECTT</title></svelte:head>

<div class="container">
	<h1 class="page-title">Browse by tools and equipment</h1>
	<p class="page-desc">Physical tools and equipment used across occupations. Select a tool to search for occupations that use it.</p>

	<div class="filter-row">
		<input class="search-input" type="text" placeholder="Filter tools..." bind:value={query} oninput={onInput} />
	</div>

	{#if loading}
		<div class="loading"><div class="spinner"></div></div>
	{:else}
		<p class="result-count">{total} tools{query ? ` matching "${query}"` : ''}</p>
		<div class="tech-grid">
			{#each items as item}
				<a href="/search/tools?q={encodeURIComponent(item.name)}" class="tech-card">
